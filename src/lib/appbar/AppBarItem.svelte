<script lang="ts">
	...
	import { addApp, apps, type AppNames, type AppWindow } from '../../stores/apps.store';
...
>
	...
	{#if isHovered}
		<p
			...
			class="absolute bottom-[4.25rem] left-1/2 -translate-x-1/2 rounded bg-neutral-800 p-0.5 px-2 py-1 text-white before:absolute before:-bottom-1 before:left-1/2 before:h-2 before:w-2 before:-translate-x-1/2 before:rotate-45 before:bg-neutral-800"
		>
			{appKey}
		</p>
	{/if}
